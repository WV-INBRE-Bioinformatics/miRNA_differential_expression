...
my $Nmature = wFILE("all_novel_mature.fa");
my $Nstar = wFILE("all_novel_star.fa");
my $Nprecursor = wFILE("all_novel_precursor.fa");

my $MDmature = wFILE("all_mirbase_detected_mature.fa");
my $MDstar = wFILE("all_mirbase_detected_star.fa");
my $MDprecursor = wFILE("all_mirbase_detected_precursor.fa");

#my $MUmature = wFILE("all_mirbase_mature_undetected.fa");
#my $MUstar = wFILE("all_mirbase_star_undetected.fa");
#my $MUprecursor = wFILE("all_mirbase_precursor_undetected.fa");
...
my $OUT = wFILE("sigNOVEL.txt");
printf $OUT ("%s", join("\n", @sigNOVEL));
close($OUT);

$OUT = wFILE("sigMATURE.txt");
printf $OUT ("%s", join("\n", @sigMATURE));
close($OUT);
...
sub wFILE
{
	...
	my ($a, $b, $d) = splice( @{[caller(1)]}, 0, 3);
	my $OUTFILE = new FileHandle(">$FILE") || die "\"InOut\" Error: wFILE(@_), Can't write to $FILE :  $!\n $d : Called from program $a($b)\n";
